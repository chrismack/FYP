...
# IO
output = common.ibm100
inputData = common.ibmbase
...
def insertTrade(dataset, washTrade, row):
    ...
    data = {
    'Timestamp': washTrade[cols[0]],
    'EventType': washTrade[cols[1]],
    'Price': washTrade[cols[2]],
    'Quantity': washTrade[cols[3]],
    'Exchange': washTrade[cols[4]],
    'Wash': washTrade[cols[5]],
    'Conditions': ['1'] * tradeLength,
    'Ticker': ['IBM'] * tradeLength,
    }
...
def generateWashTrades(rows, dataset):
    if len(rows) > 0:
        ...
        dataset = newSet
        newSet = DataFrame()
        dataset.to_csv(output, mode='w', index=False)
